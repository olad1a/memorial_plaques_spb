---
title: "Plaques Project"
author: "Lada Maksimova"
date: "04 04 2020"
...
The following research aims to explore plaquing patterns in SPb and analyze commemorative strategies of the city authorities.

### Step 1. Gathering, cleaning and ingesting the data

## Packadges loading 

```{r, warning=FALSE}

packages <- c("data.table", "stringi", "stringr", "rvest", "httr", "xml2", "readr", "tidyverse", "ggmap", "lubridate", "anytime", "ggplot2" )
...
The most extensive but not nearly exhaustive repository of information on memorial plaques of Saint Petersburg can be found in "Encyclopedia of Saint Petersburg" website with 887 pages of plaques devoted to individuals. If we refer to the website , we'll se that there's 30 pages that we will nedd to go through. To do so we will need to perform this chunk of code: 
...
Next step is to get all the files with the individual plaque pages. It will make the following extracton of the information faster. I'm also storing the unique IDs of every page, for thevariable that will come in handy in the future.
``` {r, results='hide'}
html_list <- c()
ids <- c()
...
And further on, the following chunk of code will go through the files and extract the information on the name of the 'plaqued' person(title), text inscripred on plaque, the year of plaque installation and address. As the website is fairly old and rather disorganized, I will copy the whole plaque related text into one extra variable called "description" for backup. If extracting information with following-sibling method turns out to be inefficient, I will be able to simply split "description" column into three. 
``` {r, results='hide'}
...
#column with year, architect, material
  pl_year <- xml2::xml_find_all(curr_page, "//div[@class='inner_object']/text()[preceding-sibling::br[3]][1]")
...
Finally, the dataset can be created.
```{r, results='hide'}
...
```{r}
knitr::kable(df_plaques[1:5,1:5], format="markdown")
```
As feared, following-sibling method did not work out. Unfortunatelly it resulted in numerous mistakes in the dataset. So, trying to separate the "description" column into Text, Year, Material, Presence of sculptor and Address seems to be a reasonable solution. But first, let's look at the descriprion of some of the plaques and figure the anchors that could be used as separators.
```{r, echo=FALSE}
print(plaque_content[1:3])
``` 

For extrcting plaque text it's possible to use quotation marks as a separator. For address separation "Адреса" seems to be a perfect solution to get rid of the part thats goes before the address and "Предметный" for after part. Then 
...
##Replacing NA's in Address column
```{r}
ind <- which(is.na(plaques$Address))
...
One of the cases wasn't copied properly from the website, because the page incorporated the data on 19 different plaques, placed inside and outside of the Savior on the Spilled Blood, that described the achievements of Alexander II. So, I will manually create the dataset and merge it with the main plaques data.
...
alex_df[ID == "2805524710", Description := "17 апреля 1818 года. 
        ...
        “Да встретит он обильный честью век, Да славного участник славный будет, Да чреде высокой не забудет, Святейшего из званий человек!” Жуковский. 1818 г."]
alex_df[ID == "28055247101", Description := "12 декабря 1852 года. Провозглашение Великого Князя АЛЕКСАНДРА НИКОЛАЕВИЧА Наследником Престола. 16 апреля 1841 года. Бракосочетание Наследника Цесаревича и Великого Князя АЛЕКСАНДРА НИКОЛАЕВИЧА с Великою Княжною МАРИЕЮ АЛЕКСАНДРОВНОЮ, Принцессою Гессен-Дармштадтскою"]
        их. Занятие Хивы, столицы Ханства. Взятие Кокандской крепости Махрам. Вторичное взятие штурмом города Андижан. Присоединение к владениям России Кокандского Ханства. Взятие штурмом крепости Геок-Тепе"]
alex_df[ID == "280552471017", Description := "19 февраля 1878 года. Заключение мира в С.-Стефано. Воссоединение с Россией отторженного участка Бессарабии. Присоединение Карса и Батума. Независимость Румынии, Сербии и Черногории. Освобождение Болгарии"]
alex_df[ID == "280552471018", Description := "Война за освобождение Балканских христиан. 12 апреля 1877 – 19 февраля 1878. “Я знаю, что вся Россия вместе со МНОЮ принимает живейшее участие в страданиях наших братий по вере и по происхождению... Вся Россия отзовется на МОЙ призыв, когда я сочту это нужным и честь России того потребует... Да поможет НАМ БОГ исполнить НАШЕ святое призвание” (Из речи, произнесенной ИМПЕРАТОРОМ АЛЕКСАНДРОМ II в Кремлевском Дворце 30 октября 1876 г.) Взятие Ардагана. Переправа через Дунай у Галаца и у Зимницы. Защита Баязета. Взятие Никополя. Захват Балканских походов: Ханикойского и Шипкинского. Взятие Ловчи. Поражение армии Мухтара-Паши на Аладжинских высотах. Горный Дубняк и Телиш. Деве-Бойну. Взятие штурмом Карса. Падение Плевны. Сражение при сс. Трестеника и Мечки. Переход через Балканы: Ташкисена, гор. Бугорок, Комарцы, Петричево Враждебна. Шипка. Шейново. Филиппополь. Занятие Андрианополя. Занятие Эрзерума"]
...
# Merging datasets
```{r}
#getting rid of the raw that is being substituted by 19 plaques
plaques <- plaques[-6,]
#bind
new_plaques <- rbind(plaques, alex_df, use.names=TRUE, stringsAsFactors=FALSE, sort=TRUE, fill=TRUE)

```
# Some more cleaning and preliminary analysis

Let's take a look at some basic statistic
```{r}
str(new_plaques)
```
Tidying the Person column
```{r, results='hide'}
new_plaques[, Person := as.character(Person)]
...
Extracting connections into a variable
```{r, results='hide'}
...
Making a table with top 10 connections
```{r}
new_plaques[, what_did:=as.factor(what_did)]
head(setorderv(new_plaques[, .N, by = "what_did"], "N", -1)[], 10)
```
...
```
My goal is to extract full names, birth and death dates from wiki dataset to plaques dataset. To do this I wrote a following function. It takes a lot of time to execute, so I'll keep it in comment box. And load the resulting dataset into markdown. A fully functional code is available on my Github(https://github.com/olad1a/memorial_plaques_spb)
...
The following histogtam is showing number and material of plaques in historic periods. Plaques started immerging in the second half of 18th century, when the concept of "national state" immerged. Brezhnew period showcases the highest number of plaques. It might be connected to the cultural policy of that time, when commemorative practices started playing more important role in period of stagnation and looming ideological crisis.
````{r, echo=F}
...
```
##Lenin stats
The most plaqued individual is Lenin, so it makes sense to look at him moreclosely. Patterns can be found regarding Lenins bith and death dates.
````{r, echo=FALSE}
...
This is it for now, but I am planning on continuing the research and unfolding some more interesting facts and patterns and try and build a clustering model.